...
class TestStockEntry(FrappeTestCase):
    ...
    def test_transfer_creates_two_sles(self):
        self.make_stock_entry("Receipt", qty=10, rate=500, target=self.warehouse_1)
        transfer = self.make_stock_entry(
            "Transfer", qty=3, source=self.warehouse_1, target=self.warehouse_2
        )
    ...
    def test_transfer_same_source_and_target_fails(self):
        ...
        with self.assertRaises(frappe.ValidationError):
            self.make_stock_entry(
                "Transfer", qty=3, source=self.warehouse_1, target=self.warehouse_1
            )
    ...
    def test_cancel_reverses_transfer(self):
        ...
        wh2_qty = sum(
            s.actual_qty for s in frappe.get_all(
                "Stock Ledger Entry",
